 ...
 */
@Service
public class GioneeSecurityFilter extends AbstractSecurityInterceptor implements Filter {
	//与spring-security.xml里的myFilter的属性securityMetadataSource对应，
	//其他的两个组件，已经在AbstractSecurityInterceptor定义
	@Autowired
	private GioneeSecurityMetadataSource securityMetadataSource;
	@Autowired
	private GioneeAccessDecisionManager accessDecisionManager;
	@Autowired
	private AuthenticationManager gioneeAuthenticationManager; 
	
	@PostConstruct
	public void init(){
		logger.info(" ---------------  GioneeSecurityFilter init--------------- ");
		super.setAuthenticationManager(gioneeAuthenticationManager);
	...
	private void invoke(FilterInvocation fi) throws IOException, ServletException {
		...
		logger.info(" ---------------  GioneeSecurityFilter invoke--------------- ");
